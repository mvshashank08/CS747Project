...
class mpi_inf_3dhp_dataloader(Dataset):
    ...
    def _load_data_set(self):
        ...
        with h5py.File(anno_file_path, 'r') as fp:
            ...
            print("total_image_names:", len(total_image_names))
            # add a third column of 1's to all the they keypoints. 
            # TO compensate the lack of data which specifies if the keypoint is occuled
            one = np.ones((len(total_image_names), 14,1), dtype = np.float64)
            total_kp2d = np.dstack((total_kp2d, one))
            l = len(total_kp2d)
            ...
            for index in range(l):
                ...
                self.images.append(total_image_names[index].decode())
    ...
    def __getitem__(self, index):
        try:
            ...
            print("image path:", self.images[index])
            print("Oops! Screwed up!", e)

if __name__ == '__main__':
    # Set the folder for the location of the annot file
    mpi = mpi_inf_3dhp_dataloader('../../../data/MP1/mpi_inf_3dhp/', True, [1.1, 2.0], False, 5)
    l = len(mpi)
    for _ in range(l):
        r = mpi.__getitem__(_)
        if(r != None):
            base_name = os.path.basename(r['image_name'])
            img = np.float32(r['image'])
            draw_lsp_14kp__bone(img, np.float32(r['kp_2d']))
            cv2.imwrite("./test_images/test"+str(_)+".jpg", cv2.resize(img, (512, 512), interpolation = cv2.INTER_CUBIC))
            #cv2.waitKey(0)
