...
class eval_dataloader(Dataset):
    ...
    def _load_data_set(self):
        # clk = Clock()
        path = os.path.join(self.data_folder, '*_image.png')
        ...
        for idx in range(len(self.images)):
            ...
            anno_path = os.path.join(self.data_folder, image_name + '_joints.npy')

            self.kp2ds.append(np.load(anno_path).T)
            anno_path = os.path.join(self.data_folder, image_name +'_body.pkl')
           
            with open(anno_path, 'rb') as fp:
                annos = pickle.load(fp, encoding="latin1")
...
if __name__ == '__main__':
    evl = eval_dataloader('../../../data/up-3d/up-3d', True)
    l = evl.__len__()
    print(l)
