...
# Adding the src path for the util class
sys.path.append('./src')
...
#         This is the location of the training keypoints file
        anno_file_path = os.path.join(self.data_folder, 'coco' , 'Annotation' ,'person_keypoints_train2017.json')
        ...
        def _hash_image_id_(image_id_to_info, coco_images_info):
            for image_info in coco_images_info:
                ...
                _anno = {}
                _anno['image_path'] = os.path.join(self.data_folder, 'coco', 'train2017', image_name)
                _anno['kps'] = []
                _anno['box'] = []
...
if __name__ == '__main__':
    coco = COCO2017_dataloader('../../../data/', True, [1.1, 1.5], False, False, 10, 0.1)
    torch.save(coco, 'coco_data.pt')
    l = len(coco)
    for _ in range(l):
